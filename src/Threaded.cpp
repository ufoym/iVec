...
{
#ifdef _DEBUG
    qDebug("%s thread took %f secs to get to %s", this->_name.toLatin1().data(), this->_timer.elapsed() / 1000.0, info);
#else
...
#ifdef _DEBUG
    qDebug("%s thread took %f secs to complete", this->_name.toLatin1().data(), this->_timer.elapsed() / 1000.0);
#endif
}
